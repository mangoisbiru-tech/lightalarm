...
import { Sun, Moon, Play, Pause, RotateCcw, Clock, ChevronLeft, ChevronRight, Settings, Volume2, Lightbulb, Zap, Eye, EyeOff, Cloud, CloudSun, Sunrise } from 'lucide-react';
...
const LightAlarm = () => {
  ...
  const [selectedAlarmSound, setSelectedAlarmSound] = useState('classic-alarms');

  // Dropdown selections for specific sounds
  const [selectedClassicAlarm, setSelectedClassicAlarm] = useState('alarm1.wav');
  const [selectedAmbientSound, setSelectedAmbientSound] = useState('airport-luggage-wheels-on-bricks.wav');
  const [selectedNaturalSound, setSelectedNaturalSound] = useState('forest-sounds-stereo.wav');
  ...
  const [animationIntensity, setAnimationIntensity] = useState('off'); // 'off', 'subtle', 'medium', 'dynamic'
  const [showBatteryWarning, setShowBatteryWarning] = useState(false);
  const [animationTime, setAnimationTime] = useState(0);

  // ========== BRIGHTNESS SETTING ==========
  const [maxBrightness, setMaxBrightness] = useState(70); // 50, 70, or 100
  ...
  useEffect(() => {
    ...
    if (currentHour === targetHour && currentMinute === alarmMinute && currentSecond >= 30 && !hasTriggeredSound.current) {
      ...
      console.log('Selected alarm sound:', selectedAlarmSound);
  ...
  useEffect(() => {
    if (isAlarmEnabled) {
      // Request critical permissions when user enables alarm
      requestCriticalPermissions();

      // Schedule backup notification
      scheduleAlarmNotification(alarmTime, alarmAmPm);
    }
  }, [isAlarmEnabled, alarmTime, alarmAmPm]);
  ...
  const requestCriticalPermissions = async () => {
  ...
  // ========== NOTIFICATION SYSTEM ==========
  const scheduleAlarmNotification = (alarmTime, alarmAmPm) => {
  ...
  const sendAlarmNotification = () => {
    ...
    // Disable the alarm to prevent repeated triggers (only at actual alarm time)
  ...
  }, [isRunning, animationIntensity]);

  // Battery warning logic
  useEffect(() => {
    if (animationIntensity === 'dynamic' && !showBatteryWarning) {
  ...
  const sleepThemesPage1 = {
    midnightpurple: {
      ...
      colors: ['#9d8cc7', '#8b7ab8', '#7968a9', '#67569a', '#55448b', '#43327c', '#31206d', '#1f0e5e', '#1a0a52', '#150746', '#10043a', '#0b022e'],
      duration: sleepTimer,
      animation: 'spiral'
    },
    bluesea: {
    ...
    tealdream: {
      name: 'Teal Dream',
      colors: ['#5f9ea0', '#4f8e90', '#3f7e80', '#2f6e70', '#1f5e60', '#1a5459', '#154a52', '#10404b', '#0b3644', '#062c3d', '#032236', '#01182f'],
      duration: sleepTimer,
      animation: 'waves'
    }
  };
  ...
  const sleepThemesPage2 = {
    ...
    indigoabyss: {
      name: 'Indigo Abyss',
      colors: ['#4b0082', '#420070', '#39005e', '#30004c', '#27003a', '#1e0028', '#150016', '#0f0012', '#0a000e', '#05000a', '#020006', '#000000'],
      duration: sleepTimer,
      animation: 'spiral'
    },
    neonsnake: {
  ...
  const calculateBrightness = () => {
    if (currentMode === 'wake') {
      // Use maxBrightness setting (50, 70, or 100) to limit how bright it gets
      const maxBrightnessValue = maxBrightness / 100; // Convert percentage to 0-1 range

      if (progress <= 50) {
        // First half: gradually increase to 70% of max
        return (progress / 50) * 0.7 * maxBrightnessValue; // 0% → 70% of max
      } else if (progress < 100) {
        // Second half: gradually increase from 70% to 100% of max
        return (0.7 + ((progress - 50) / 50) * 0.3) * maxBrightnessValue; // 70% → 100% of max
      } else {
        return maxBrightnessValue; // Stay at max setting
      }
    } else {
  ...
  const getDynamicBrightnessVariation = (animationType, time) => {
    // CANDLE FLICKER MODE - When comfort light is enabled
    if (isComfortLightEnabled && isComfortLightRunning && currentMode === 'sleep') {
      const t = time / 1000;

      // Realistic candle flicker with multiple frequency components
      // Primary flicker (fast, erratic flame dance)
      const fastFlicker = Math.sin(t * 8 + Math.sin(t * 3) * 0.5) * 0.15;

      // Secondary wobble (medium speed, flame sway)
      const mediumWobble = Math.sin(t * 3.5 + Math.cos(t * 2.1) * 0.3) * 0.12;

      // Slow drift (gentle breathing of flame)
      const slowDrift = Math.sin(t * 0.8) * 0.08;

      // Random-like spikes (occasional bright flickers)
      const spikes = Math.sin(t * 12.7) * Math.cos(t * 9.3) * 0.10;

      // Combine all components for realistic candle effect
      return fastFlicker + mediumWobble + slowDrift + spikes;
    }

    // REGULAR DYNAMIC BRIGHTNESS - For sleep animations
  ...
  const naturalSoundsLibrary = [
    { displayName: "Forest Stereo", fileName: "forest-sounds-stereo.wav" },
    { displayName: "Summer Forest", fileName: "forest-summe.wav" },
    { displayName: "Relaxing Sea", fileName: "relaxing-sea.m4a" },
    { displayName: "Deep Water Loop", fileName: "underwater-deep-water-loop.flac" }
  ];
  ...
  const [naturalSoundsPlaylist, setNaturalSoundsPlaylist] = useState([]);
  ...
  useEffect(() => {
    ...
    setNaturalSoundsPlaylist(shuffle(naturalSoundsLibrary));
  ...
  const getNextTrack = (category) => {
    ...
    switch(category) {
      ...
      case 'ambience':
        ...
        break;
      case 'natural-sounds':
        playlist = naturalSoundsPlaylist;
        setPlaylist = setNaturalSoundsPlaylist;
        library = naturalSoundsLibrary;
        break;
      default:
  ...
  const alarmSounds = {
    ...
    // 4. Natural Sounds (from R2)
    'natural-sounds': {
      name: 'Natural Sounds',
      description: 'Rain sounds and ocean waves',
      category: 'Natural Sounds',
      isR2Audio: true,
      folder: 'Natural_Sound',
      getTrack: () => getNextTrack('natural-sounds')
    }
  };
  ...
  const playR2Audio = (soundConfig) => {
    ...
    // Check if we should use dropdown selection
    if (selectedAlarmSound === 'classic-alarms') {
      fileName = selectedClassicAlarm;
      displayName = classicAlarmLibrary.find(t => t.fileName === selectedClassicAlarm)?.displayName || fileName;
    } else if (selectedAlarmSound === 'ambient-sounds') {
      fileName = selectedAmbientSound;
      displayName = ambienceLibrary.find(t => t.fileName === selectedAmbientSound)?.displayName || fileName;
    } else if (selectedAlarmSound === 'natural-sounds') {
      fileName = selectedNaturalSound;
      displayName = naturalSoundsLibrary.find(t => t.fileName === selectedNaturalSound)?.displayName || fileName;
    } else {
      // Use random track for other categories
      const track = soundConfig.getTrack();
      if (!track) {
  ...
  const playAlarmSound = async () => {
    console.log('🔊 Starting alarm audio:', selectedAlarmSound);
    ...
    try {
      ...
      // Get the selected alarm sound
      let alarmConfig = alarmSounds[selectedAlarmSound] || alarmSounds['classic-alarms'];
  ...
  const AnimatedBackground = ({ animation, color, brightness }) => {
    ...
    if (animation === 'paintball') {
      // Neon Paintball - paintballs shooting and bursting one after another
      const time = animationTime / 1000;
      const splashes = [];
      ...
      // Timing: 1 splash every 1.5 seconds, stays for 3 seconds total
      const splashInterval = 1.5;
      const splashLifespan = 3;
      const maxVisibleSplashes = 8; // Keep up to 8 splashes visible at once

      for (let i = 0; i < maxVisibleSplashes; i++) {
        const splashStartTime = i * splashInterval;
        const timeSinceStart = time - splashStartTime;

        // Only show if it's time for this splash to appear
        if (timeSinceStart < 0) continue;

        const cycleTime = timeSinceStart % (splashInterval * maxVisibleSplashes);

        // Skip if this splash has already faded out
        if (cycleTime > splashLifespan) continue;
        ...
        if (cycleTime < 0.15) {
          // Phase 1: Paintball impact & instant burst (0-0.15s)
          burstProgress = cycleTime / 0.15;
          scale = burstProgress * 1.2; // Quick expansion
          opacity = burstProgress;
        } else if (cycleTime < 0.3) {
          // Phase 2: Settle to final size (0.15-0.3s)
          burstProgress = 1;
          const settleProgress = (cycleTime - 0.15) / 0.15;
          scale = 1.2 - (settleProgress * 0.2); // Settle from 1.2 to 1.0
          opacity = 1;
        } else if (cycleTime < splashLifespan - 0.5) {
          // Phase 3: Hold (0.3s - 2.5s)
          burstProgress = 1;
          scale = 1;
          opacity = 0.9;
        } else {
          // Phase 4: Fade out (2.5s - 3s)
          burstProgress = 1;
          scale = 1;
          opacity = 0.9 * (1 - (cycleTime - (splashLifespan - 0.5)) / 0.5);
        }

        // Position (pseudo-random based on index)
        const positionSeed = Math.floor(timeSinceStart / (splashInterval * maxVisibleSplashes));
        const x = ((17 + (i + positionSeed * 7) * 23) % 85) + 7.5; // 7.5-92.5%
        const y = ((31 + (i + positionSeed * 5) * 19) % 80) + 10; // 10-90%

        // Size variation
        const size = 180 + ((i + positionSeed) % 3) * 70; // 180-320px

        // Color
        const color = paintColors[(i + positionSeed) % paintColors.length];

        // Number of splatter particles based on burst progress
        const numSplatters = 12;

        splashes.push(
          <div
            key={`splash-${i}-${positionSeed}`}
            className="absolute"
            style={{
              left: `${x}%`,
              top: `${y}%`,
          ...
          >
            ...
            <div
              style={{
                width: '100%',
                height: '100%',
                background: `radial-gradient(circle, ${color}ff 0%, ${color}dd 25%, ${color}88 50%, ${color}44 75%, transparent 100%)`,
                borderRadius: '43% 57% 48% 52% / 55% 49% 51% 45%',
                filter: 'blur(3px)',
                boxShadow: `0 0 50px ${color}cc, 0 0 100px ${color}66, 0 0 150px ${color}33`,
              }}
            />

            {/* Burst splatter particles */}
            {[...Array(numSplatters)].map((_, splatterIdx) => {
              const angle = (splatterIdx / numSplatters) * 360 + (i * 15);
              const distance = 45 + (splatterIdx % 3) * 15;
              const splatterSize = 15 + (splatterIdx % 4) * 8;
              const splatterDistance = distance * burstProgress;

              return (
                <div
                  key={`splatter-${splatterIdx}`}
                  style={{
                    position: 'absolute',
                    left: '50%',
                    top: '50%',
                    ...
                    background: `radial-gradient(circle, ${color}ff 0%, ${color}cc 40%, transparent 100%)`,
                    borderRadius: splatterIdx % 2 === 0 ? '50%' : '40% 60% 50% 50%',
                    transform: `translate(-50%, -50%) rotate(${angle}deg) translateY(-${splatterDistance}px)`,
                    filter: 'blur(1px)',
                    boxShadow: `0 0 ${splatterSize}px ${color}88`,
                    opacity: burstProgress,
                  }}
                />
              );
    ...
    if (animation === 'neontree') {
      // Neon Tree - glowing tree with pulsing neon green leaves and brown trunk
      const time = animationTime / 1000;
      const branches = [];

      // Tree trunk (neon brown/orange)
      const trunkGlow = 0.6 + Math.sin(time * 0.5) * 0.2;

      // Generate branching structure
      const createBranch = (startX, startY, angle, length, depth, color, glowIntensity) => {
        ...
        if (depth > 1) {
          // Left branch
          branchArray.push(...createBranch(
            endX, endY,
            angle - 0.5, // Angle left
            length * 0.7,
            depth - 1,
            color,
            glowIntensity * 0.9
      ...
      // Main trunk
      const trunkBranches = createBranch(
        50, 95, // Start at bottom center
        -Math.PI / 2, // Grow upward
        35, // Length
        3, // Depth (3 levels of branches)
        '#ff6600', // Neon orange/brown trunk
        trunkGlow
      );

      // Foliage branches (neon green)
      const leafPulse = 0.7 + Math.sin(time * 0.8) * 0.3;
      const foliageBranches = createBranch(
        50, 60, // Start higher up
        -Math.PI / 2,
        25,
        4, // More depth for leaves
        '#39ff14', // Neon green
        leafPulse
      );

      // Render all branches
      [...trunkBranches, ...foliageBranches].forEach((branch, idx) => {
        branches.push(
          <div
            key={`branch-${idx}`}
            className="absolute"
            style={{
              left: `${branch.startX}%`,
              top: `${branch.startY}%`,
              width: `${Math.sqrt(Math.pow(branch.endX - branch.startX, 2) + Math.pow(branch.endY - branch.startY, 2))}%`,
              height: `${branch.width}px`,
              background: `linear-gradient(to right, ${branch.color}ff, ${branch.color}dd)`,
              transformOrigin: 'left center',
              transform: `rotate(${Math.atan2(branch.endY - branch.startY, branch.endX - branch.startX)}rad)`,
              opacity: branch.glowIntensity,
              boxShadow: `0 0 ${branch.width * 3}px ${branch.color}aa, 0 0 ${branch.width * 6}px ${branch.color}66`,
              filter: 'blur(0.5px)',
              pointerEvents: 'none',
            }}
          />
      ...
      // Add glowing leaf particles
      const leaves = [];
      for (let i = 0; i < 25; i++) {
        const leafX = 45 + Math.sin(time * 0.5 + i * 0.8) * 8;
        const leafY = 20 + (i % 5) * 8 + Math.cos(time * 0.6 + i) * 3;
        const leafPulse = 0.5 + Math.sin(time * 1.2 + i * 0.5) * 0.4;
        const leafSize = 4 + (i % 3) * 2;
        ...
        leaves.push(
          <div
            ...
            style={{
              ...
              width: `${leafSize}px`,
              height: `${leafSize}px`,
              background: `radial-gradient(circle, #39ff14ff, #39ff1466)`,
              borderRadius: '50%',
              opacity: leafPulse,
              boxShadow: `0 0 ${leafSize * 2}px #39ff14cc, 0 0 ${leafSize * 4}px #39ff1488`,
              filter: 'blur(0.3px)',
              pointerEvents: 'none',
            }}
          />
        ...
        <div className="absolute inset-0 overflow-hidden" style={{ ...baseStyle, background: '#000000' }}>
          ...
          <div
            ...
            style={{
              ...
              opacity: 0.5 + Math.sin(time * 0.4) * 0.2,
              pointerEvents: 'none',
            }}
          />
        </div>
    ...
    return (
      <div className="min-h-screen flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 min-h-screen flex flex-col">
          <div className="p-6">
            {/* Header Card */}
            <div className="bg-white/80 backdrop-blur-xl rounded-3xl p-6 mb-6 shadow-xl border-2 border-white/60">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h1 className="text-2xl font-bold text-slate-800" style={{ fontFamily: 'Georgia, serif' }}>Settings</h1>
                    <p className="text-xs text-slate-600">Customize your alarm experience</p>
                  </div>
                </div>
                <button
                  onClick={() => setShowSettings(false)}
                  className="w-12 h-12 rounded-2xl bg-slate-100 flex items-center justify-center hover:bg-slate-200 transition-all shadow-md"
                >
            ...
            {/* Section 1: Alarm Settings */}
            <div className="bg-white/70 backdrop-blur-xl rounded-3xl p-6 mb-6 shadow-lg border-2 border-white/60">
              <div className="flex items-center space-x-3 mb-4">
                ...
                <div>
                  <h2 className="text-lg font-bold text-slate-800" style={{ fontFamily: 'Georgia, serif' }}>Alarm Settings</h2>
                  <p className="text-xs text-slate-600">Configure your wake-up alarm sound</p>
                </div>
              </div>

              <div className="space-y-3">
                {Object.entries(alarmSounds).filter(([key, sound]) => !sound.hidden).map(([key, sound]) => (
                  <div key={key}>
                    <div
                      ...
                      onClick={() => setSelectedAlarmSound(key)}
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          <h3 className="font-semibold text-slate-800">{sound.name}</h3>
                          <p className="text-sm text-slate-600">{sound.description}</p>
                        </div>
                        <div className="flex items-center space-x-2">
                          <button
                            className={`w-8 h-8 rounded-full flex items-center justify-center hover:scale-110 transition-all duration-200 shadow-lg ${
                              previewingSound === key
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              console.log(`Testing audio for ${sound.name}...`);
                              previewSound(key);
                            }}
                            title={previewingSound === key ? "Stop preview" : "Play preview"}
                          >
                            <span className={previewingSound === key ? "text-red-400" : "text-green-400"}>
                              {previewingSound === key ? "⏸" : "▶"}
                            </span>
                          </button>
                          {selectedAlarmSound === key && (
                            <div className="w-6 h-6 rounded-full bg-green-500 flex items-center justify-center shadow-lg shadow-green-500/30 animate-pulse">
                    ...
                    {/* Dropdown for Classic Alarm */}
                    {selectedAlarmSound === 'classic-alarms' && key === 'classic-alarms' && (
                      <div className="mt-2 ml-4 p-3 bg-blue-500/20 rounded-lg border border-blue-400/40">
                        <label className="text-sm font-medium mb-2 block text-white">Select Alarm Sound:</label>
                        <select
                          value={selectedClassicAlarm}
                          onChange={(e) => setSelectedClassicAlarm(e.target.value)}
                          className="w-full bg-slate-700 text-white rounded-lg px-3 py-2 border-2 border-blue-400/60 focus:bg-slate-600 focus:border-blue-300 focus:outline-none shadow-md"
                          onClick={(e) => e.stopPropagation()}
                        >
                          {classicAlarmLibrary.map((track) => (
                            <option key={track.fileName} value={track.fileName} className="bg-slate-700">
                              {track.displayName}
                            </option>
                          ))}
                        </select>
                    ...
                    {/* Dropdown for Ambient Sounds */}
                    {selectedAlarmSound === 'ambient-sounds' && key === 'ambient-sounds' && (
                      <div className="mt-2 ml-4 p-3 bg-blue-500/20 rounded-lg border border-blue-400/40">
                        <label className="text-sm font-medium mb-2 block text-white">Select Ambient Sound:</label>
                        <select
                    ...
                    {/* Dropdown for Natural Sounds */}
                    {selectedAlarmSound === 'natural-sounds' && key === 'natural-sounds' && (
                      <div className="mt-2 ml-4 p-3 bg-blue-500/20 rounded-lg border border-blue-400/40">
                        <label className="text-sm font-medium mb-2 block text-white">Select Natural Sound:</label>
                        <select
                          value={selectedNaturalSound}
                          onChange={(e) => setSelectedNaturalSound(e.target.value)}
                          className="w-full bg-slate-700 text-white rounded-lg px-3 py-2 border-2 border-blue-400/60 focus:bg-slate-600 focus:border-blue-300 focus:outline-none shadow-md"
                          onClick={(e) => e.stopPropagation()}
                        >
                          {naturalSoundsLibrary.map((sound) => (
                            <option key={sound.fileName} value={sound.fileName} className="bg-slate-700">
                              {sound.displayName}
                            </option>
                          ))}
                        </select>
            ...
            <div className="bg-white/70 backdrop-blur-xl rounded-3xl p-6 mb-6 shadow-lg border-2 border-white/60">
              <div className="flex items-center space-x-3 mb-4">
                ...
                <div>
                  <h2 className="text-lg font-bold text-slate-800" style={{ fontFamily: 'Georgia, serif' }}>Brightness</h2>
                  <p className="text-xs text-slate-600">Set maximum screen brightness level</p>
                </div>
              </div>
              ...
              <div className="p-4 bg-white/10 rounded-lg">
                <div className="grid grid-cols-3 gap-3 mb-3">
                  {[
                    { value: 50, label: '50%', desc: 'Gentle' },
                    { value: 70, label: '70%', desc: 'Recommended', recommended: true },
                    { value: 100, label: '100%', desc: 'Maximum' }
                  ].map((option) => (
                    <button
                      key={option.value}
                      onClick={() => setMaxBrightness(option.value)}
                      className={`p-3 rounded-lg text-center transition-all relative ${
                        maxBrightness === option.value
                          ? 'bg-yellow-500/30 border-2 border-yellow-400/60 shadow-md'
                ...
                <div className="p-3 bg-blue-500/10 rounded-lg">
                  <p className="text-xs text-slate-700">
                    💡 <strong>70%</strong> is recommended - bright enough for effective wake-up without being harsh. <strong>50%</strong> for gentler wake. <strong>100%</strong> for maximum intensity.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/70 backdrop-blur-xl rounded-3xl p-6 mb-6 shadow-lg border-2 border-white/60">
              <div className="flex items-center space-x-3 mb-4">
                ...
                <div>
                  <h2 className="text-lg font-bold text-slate-800" style={{ fontFamily: 'Georgia, serif' }}>System Permissions</h2>
                  <p className="text-xs text-slate-600">Ensure reliable alarm functionality on Android</p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div className="p-3 bg-white/10 rounded-lg">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs opacity-70">Required for backup alerts</p>
                    </div>
                    <div className="flex items-center space-x-2">
                      <div className={`w-3 h-3 rounded-full ${
                        permissionStatus.notifications === 'granted' ? 'bg-green-500' :
                        permissionStatus.notifications === 'denied' ? 'bg-red-500' : 'bg-yellow-500'
                      }`} />
                      <span className="text-xs capitalize">{permissionStatus.notifications}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-3 bg-white/10 rounded-lg">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs opacity-70">Prevents Android from killing alarm</p>
                    </div>
                    <div className="flex items-center space-x-2">
                      <div className={`w-3 h-3 rounded-full ${
                        permissionStatus.batteryOptimization === 'user-guided' ? 'bg-green-500' :
                        permissionStatus.batteryOptimization === 'declined' ? 'bg-red-500' : 'bg-yellow-500'
                      }`} />
                      <span className="text-xs capitalize">{permissionStatus.batteryOptimization.replace('-', ' ')}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-3 bg-white/10 rounded-lg">
                  <div className="flex items-center justify-between">
                    <div>
                      <span className="font-medium">Motion Sensors</span>
                      <p className="text-xs opacity-70">For future adaptive wake (V2.0)</p>
                    </div>
                    <div className="flex items-center space-x-2">
                      <div className={`w-3 h-3 rounded-full ${
                        permissionStatus.motionSensors === 'granted' || permissionStatus.motionSensors === 'available' ? 'bg-green-500' :
                        permissionStatus.motionSensors === 'denied' ? 'bg-red-500' : 'bg-yellow-500'
                      }`} />
                      <span className="text-xs capitalize">{permissionStatus.motionSensors}</span>
                    </div>
                  </div>
                </div>

                <button
                  onClick={requestCriticalPermissions}
                  className="w-full py-2 bg-blue-500/20 hover:bg-blue-500/30 text-blue-200 rounded-lg text-sm font-medium transition-all"
                >
            ...
            <div className="bg-white/70 backdrop-blur-xl rounded-3xl p-6 mb-6 shadow-lg border-2 border-white/60">
              <div className="flex items-center space-x-3 mb-4">
                ...
                <div>
                  <h2 className="text-lg font-bold text-slate-800" style={{ fontFamily: 'Georgia, serif' }}>Visual Effects</h2>
                  <p className="text-xs text-slate-600">Control animation intensity and battery usage</p>
                </div>
              </div>

              <div className="p-4 bg-white/10 rounded-lg">
                <div className="mb-4">
                  <h3 className="text-sm font-medium mb-3">Animation Intensity</h3>
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      { key: 'off', label: 'Off', desc: 'No animations (saves battery)' },
                      { key: 'subtle', label: 'Subtle', desc: 'Gentle, minimal movement' },
                      { key: 'medium', label: 'Medium', desc: 'Balanced visual richness' },
                      { key: 'dynamic', label: 'Dynamic', desc: 'Full cinematic experience' }
                    ].map((option) => (
                      <button
                        key={option.key}
                        onClick={() => setAnimationIntensity(option.key)}
                        className={`p-3 rounded-lg text-left transition-all ${
                          animationIntensity === option.key
                ...
                <div className="text-xs opacity-70 bg-purple-500/20 p-3 rounded mb-4">
                  <strong>Nendo-Inspired Effects:</strong>
                  <br />• <strong>Sunrise:</strong> Breathing pattern + circular wave expansion + floating particles
                  <br />• <strong>Ocean:</strong> Horizontal wave movement + ripple effects + ocean rhythm
                  <br />• <strong>Aurora:</strong> Vertical curtain movement + shifting color bands + gentle flickers
                  <br />• <strong>Nature:</strong> Dappled light effect + swaying motion + branch shadows
                </div>

                {showBatteryWarning && (
                  <div className="p-3 bg-orange-500/20 rounded-lg border border-orange-400/30">
                    <p className="text-sm text-orange-200">
                      <strong>⚡ Battery Note:</strong> Dynamic animations provide the richest visual experience but may use more battery.
                      For extended use, consider Medium or Subtle settings.
                    </p>
                  </div>
                )}
              </div>
            ...
            {/* Section 5: About & Attributions */}
            <div className="bg-white/70 backdrop-blur-xl rounded-3xl p-6 shadow-lg border-2 border-white/60">
              <div className="flex items-center space-x-3 mb-4">
                <div className="w-10 h-10 bg-gradient-to-br from-green-400 to-teal-500 rounded-xl flex items-center justify-center shadow-md">
                  <span className="text-2xl">ℹ️</span>
                </div>
                <div>
                  <h2 className="text-lg font-bold text-slate-800" style={{ fontFamily: 'Georgia, serif' }}>About & Attributions</h2>
                  <p className="text-xs text-slate-600">LightAlarm story and credits</p>
                </div>
              </div>

              {/* About LightAlarm Story */}
              <div className="p-4 bg-gradient-to-br from-blue-50 to-cyan-50 rounded-lg mb-4 border border-blue-200/40">
                <h3 className="text-sm font-bold text-slate-800 mb-2">💡 The LightAlarm Story</h3>
                <p className="text-xs text-slate-700 leading-relaxed">
                  LightAlarm was born from a simple frustration: traditional alarms are harsh and jarring.
                  We believe waking up should be gentle and natural, like sunrise. This app combines light therapy
                  principles with beautiful animations to create a peaceful wake-up experience. From sky-cloud designs
                  to neon themes, every element is crafted to make your mornings better. Built with care for people
                  who want to wake up feeling refreshed, not startled.
                </p>
              </div>

              {/* Sound Attributions - Collapsible */}
              <div className="bg-white/50 rounded-lg border border-slate-300/40 overflow-hidden">
                <button
                  onClick={() => setShowAttributions(!showAttributions)}
                  className="w-full p-2 flex items-center justify-between hover:bg-white/70 transition-all"
                >
                  <div className="flex items-center space-x-2">
                    <span className="text-sm">🎵</span>
                    <span className="text-xs font-medium text-slate-700">Sound Attributions</span>
                  </div>
                  <div className={`w-3 h-3 rounded-full border-2 transition-all ${showAttributions ? 'bg-slate-500 border-slate-500' : 'border-slate-400'}`} />
                </button>

                {showAttributions && (
                  <div className="p-2 bg-white/30 border-t border-slate-300/40">
                    <p className="text-xs text-slate-600 mb-2">
                      Sounds from <strong>Freesound.org</strong> (CC License):
                    </p>
                    <div className="text-xs text-slate-600">
                      <div className="flex flex-wrap gap-1">
                        {[
                          '4team', 'fons', 'kontraamusic', 'Disquantic', 'Seth_Makes_Sounds',
                          'OnlyTheGhosts', 'astro_denticle', 'FlavioConcini', 'knzz.io',
                          'Migfus20', 'nlux', 'Tempouser', 'djfroyd', 'univ_lyon3',
                          'deadmanswill', 'Tomlija', 'kyles', 'Vetter', 'Balin',
                          'klankbeeld', 'klankbeeld', 'mrspivey', 'AMPI_sound', 'hmmm101'
                        ].map((artist, idx) => (
                          <span key={idx} className="inline-block bg-slate-200 px-1.5 py-0.5 rounded text-xs">
                            {artist}
                          </span>
                        ))}
                      </div>
              ...
              <div className="mt-3 text-center">
                <p className="text-xs text-slate-500">LightAlarm v1.0</p>
              </div>
            </div>
  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden">
      ...
      <div className="relative z-10 min-h-screen flex flex-col pb-24"
           style={{
             background: !isRunning ? (currentMode === 'wake' ? 'linear-gradient(to bottom, #e0f2fe 0%, #bae6fd 90%, #fed7aa 100%)' : 'linear-gradient(135deg, #1e3a8a 0%, #1e40af 30%, #3b82f6 60%, #60a5fa 90%, #ff6b35 100%)') : 'transparent'
           }}
        ...
        <div className="p-6">
          {currentMode === 'wake' ? (
            <div className="relative mb-8 overflow-hidden rounded-3xl bg-gradient-to-b from-blue-300 via-cyan-200 to-orange-200 p-5 border-2 border-white/70 backdrop-blur-sm shadow-lg">
              <div className="relative flex items-center justify-between mb-2">
                ...
                <div className="flex gap-2">
                  ...
                  >
                    <Cloud className="w-4 h-4 text-slate-800" />
                  </button>
                  <button
              ...
              {/* Date Display */}
              <div className="relative text-sm text-slate-700 flex items-center justify-between">
            ...
            <div className="relative mb-8 overflow-hidden rounded-3xl bg-gradient-to-b from-indigo-500 via-purple-500 to-indigo-600 p-5 border-2 border-white/70 backdrop-blur-sm shadow-lg">
              <div className="relative flex items-center justify-between mb-2">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-gradient-to-br from-purple-300 to-indigo-400 rounded-full flex items-center justify-center border-3 border-white shadow-lg">
                    <Moon className="w-6 h-6 text-white" />
                  </div>
                  <div>
                    <h1 className="text-xl font-semibold text-white drop-shadow-sm">LightAlarm</h1>
                    <p className="text-xs text-purple-100">Sleep Peacefully</p>
                  </div>
                </div>
                <div className="flex gap-2">
                  <button
              ...
              {/* Date Display */}
              <div className="relative text-sm text-purple-100 flex items-center justify-between">
                <span>{currentTime.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}</span>
                <span>{currentTime.toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit' })}</span>
              </div>
            </div>
          ...
          {currentMode === 'sleep' && (
            <div className="mb-4 space-y-2">
              ...
              <div className="p-3 bg-white/10 rounded-lg border-l-2 border-red-400/60">
                {/* Candle Light Toggle */}
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center space-x-2">
          ...
          {currentMode === 'sleep' && !isComfortLightEnabled && (
            <div className="p-3 bg-white/10 rounded-lg shadow-lg border border-white/20">
              <h3 className="text-sm font-bold mb-2 tracking-wide italic text-cyan-300" style={{ fontFamily: 'Georgia, serif', textShadow: '0 0 10px rgba(103, 232, 249, 0.5)' }}>Sleep Timer</h3>
              <div className="grid grid-cols-2 gap-1.5">
                ...
                >
                  <span className="text-xs font-medium">All Night</span>
                </button>
              </div>
            </div>
          )}
          ...
          {currentMode === 'sleep' && !isComfortLightEnabled && (
            <div className="p-3 bg-white/10 rounded-lg mt-2 shadow-lg border border-white/20">
              <h3 className="text-sm font-bold mb-2 tracking-wide italic text-cyan-300" style={{ fontFamily: 'Georgia, serif', textShadow: '0 0 10px rgba(103, 232, 249, 0.5)' }}>Sleep Theme</h3>
                ...
                {Object.entries(getCurrentThemes()).map(([key, theme]) => (
                  ...
                  >
                    ...
                    <div className="text-center">
                      <span className="text-xs font-medium block leading-tight text-white">{theme.name}</span>
                    </div>
                  </button>
                ))}
              </div>
          ...
          {currentMode === 'wake' && (
            <div className="mb-6 space-y-4">
              {/* 1. Main Time Card - Sky Cloud Design */}
              <div className="relative">
                <div className="absolute inset-0 bg-gradient-to-br from-blue-200 to-orange-200 rounded-[2.5rem] blur-xl opacity-30"></div>

                <div className="relative bg-white/80 backdrop-blur-2xl rounded-[2.5rem] p-8 border-2 border-blue-200/60 shadow-xl overflow-hidden">
                  {/* Blue sky gradient at top */}
                  <div className="absolute top-0 left-0 right-0 h-32 bg-gradient-to-b from-blue-100/50 via-cyan-100/30 to-transparent"></div>

                  {/* Orange horizon glow at bottom */}
                  <div className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-orange-100/40 to-transparent"></div>

                  {/* White clouds floating */}
                  <div className="absolute top-6 right-10 w-12 h-8 bg-white/50 backdrop-blur-sm rounded-full"></div>
                  <div className="absolute top-8 right-8 w-10 h-6 bg-white/60 backdrop-blur-sm rounded-full"></div>
                  <div className="absolute top-12 left-8 w-14 h-9 bg-white/40 backdrop-blur-sm rounded-full"></div>
                  <div className="absolute top-14 left-10 w-12 h-7 bg-white/50 backdrop-blur-sm rounded-full"></div>
                  ...
                  <div className="relative z-10">
                    <div className="flex items-center justify-between mb-6">
                      ...
                      <div className="relative">
                        <div className="absolute inset-0 bg-blue-400 rounded-full blur-md opacity-40"></div>
                        <button
                          onClick={() => setIsAlarmEnabled(!isAlarmEnabled)}
                          className={`relative w-14 h-7 rounded-full border-2 border-white/60 transition-all ${
                            isAlarmEnabled ? 'bg-gradient-to-r from-blue-400 to-cyan-400' : 'bg-gray-300'
                          }`}
                        >
                          <div className={`w-5 h-5 bg-white rounded-full absolute top-0.5 shadow-sm transition-all ${
                            isAlarmEnabled ? 'right-1' : 'left-1'
                          }`}></div>
                        </button>
                      </div>
                    </div>
                    ...
                    {isAlarmEnabled && (
                      <div className="flex items-baseline justify-center gap-4 mb-2">
                        <select
                          ...
                          onChange={(e) => {
                            const [_, minute] = alarmTime.split(':');
                    ...
                    <div className="text-center text-sm text-blue-600 mb-6">
                      {currentTime.toLocaleDateString('en-US', { weekday: 'long' })} morning
                    </div>

                    {/* Info Box with gradient */}
                    {isAlarmEnabled && (
                      <div className="relative">
                        <div className="absolute inset-0 bg-gradient-to-br from-blue-200 to-orange-200 rounded-2xl blur-sm opacity-60"></div>
                        <div className="relative bg-gradient-to-br from-blue-100/70 via-cyan-100/70 to-orange-100/70 backdrop-blur-xl rounded-2xl p-5 border-2 border-white/60">
                          ...
                          <div className="space-y-3 text-sm">
                            <div className="flex items-start gap-3">
                              <div className="w-10 h-10 bg-gradient-to-br from-purple-300 to-indigo-300 rounded-full flex items-center justify-center shadow-sm backdrop-blur-sm flex-shrink-0">
                                <span className="text-xs font-semibold text-white">1️⃣</span>
                              </div>
                              <div className="text-blue-900">
                                <div className="font-semibold">Choose your wake up theme</div>
                                <div className="text-xs text-blue-700">Select colors that energize you</div>
                              </div>
                            </div>
                            <div className="flex items-start gap-3">
                              <div className="w-10 h-10 bg-gradient-to-br from-orange-300 to-amber-300 rounded-full flex items-center justify-center shadow-sm backdrop-blur-sm flex-shrink-0">
                                <span className="text-xs font-semibold text-white">2️⃣</span>
                              </div>
                              <div className="text-blue-900">
                                <div className="font-semibold">20 min before alarm</div>
                                <div className="text-xs text-blue-700">Screen gradually brightens (sunrise simulation)</div>
                              </div>
                            </div>
                            <div className="flex items-start gap-3">
                              <div className="w-10 h-10 bg-gradient-to-br from-yellow-300 to-orange-400 rounded-full flex items-center justify-center shadow-sm backdrop-blur-sm flex-shrink-0">
                                <span className="text-xs font-semibold text-white">3️⃣</span>
                              </div>
                              <div className="text-blue-900">
                                <div className="font-semibold">At alarm time</div>
                                <div className="text-xs text-blue-700">Full brightness + alarm sound plays</div>
                              </div>
                            </div>
                          </div>
                        </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-6 pl-4">
          ...
          <div className="relative mb-4">
            <div className={`w-32 h-32 rounded-full border-4 ${currentMode === 'wake' ? 'border-yellow-300' : 'border-yellow-300/40'}`}
                 style={currentMode === 'sleep' ? { boxShadow: '0 0 20px rgba(250, 204, 21, 0.6)' } : {}}>
              <div
                className={`w-full h-full rounded-full border-4 transition-all duration-300 ${currentMode === 'wake' ? 'border-yellow-500' : 'border-yellow-400'}`}
                style={{
                  ...
                  ...(currentMode === 'sleep' ? { boxShadow: '0 0 25px rgba(250, 204, 21, 0.8)' } : {})
                }}
              />
            </div>
            <div className="absolute inset-0 flex items-center justify-center">
              <span className={`text-xl font-bold ${currentMode === 'wake' ? 'text-yellow-600' : 'text-yellow-300'}`}
                    style={currentMode === 'sleep' ? { textShadow: '0 0 15px rgba(250, 204, 21, 0.8)' } : {}}>{Math.round(progress)}%</span>
            </div>
          </div>

          {/* Status Text */}
          <div className="text-center mb-4">
            <h2 className={`text-xl font-semibold mb-2 ${currentMode === 'wake' ? 'text-blue-900' : 'text-white'}`}>
              {currentMode === 'wake' ? 'Wake Up Simulation' : 'Sleep Simulation'}
            </h2>
            <p className={`${currentMode === 'wake' ? 'text-blue-700' : 'text-white opacity-80'}`}>
              {config.name} • {currentMode === 'sleep' ? `${sleepTimer}min` : `${config.duration}s`} duration
            </p>
            {isRunning && (
              <p className={`text-sm mt-2 ${currentMode === 'wake' ? 'text-blue-600' : 'text-white opacity-60'}`}>
          ...
          <div className="flex justify-center space-x-4">
            {!isRunning ? (
              ...
              >
                ...
                <div className="flex flex-col items-center">
                  <span className="font-semibold">Start {currentMode === 'wake' ? 'Wake Up' : 'Sleep'}</span>
            ...
            <button
              ...
              className={`flex items-center space-x-2 px-4 py-3 rounded-xl transition-all ${
                currentMode === 'wake'
                  ? 'bg-white/50 hover:bg-white/70 text-blue-900'
        ...
        {/* Bottom Info */}
        <div className={`p-6 text-center ${currentMode === 'wake' ? 'text-slate-700' : 'text-white/60'}`}>
