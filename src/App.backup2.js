...
const LightAlarm = () => {
  ...
  const [selectedNaturalSound, setSelectedNaturalSound] = useState('rain-sounds');
  ...
  const playAlarmSound = async () => {
    ...
    try {
      ...
      // If Natural Sounds is selected, get the actual sound from dropdown
      if (selectedAlarmSound === 'natural-sounds') {
        alarmConfig = alarmSounds[selectedNaturalSound];
      }
  ...
  const AnimatedBackground = ({ animation, color, brightness }) => {
    ...
    if (animation === 'lightning') {
      ...
      const [lastFlash, setLastFlash] = React.useState(Date.now());
      const [flashActive, setFlashActive] = React.useState(false);
      const [bolt, setBolt] = React.useState(null);
      
      React.useEffect(() => {
        const checkFlash = () => {
          const now = Date.now();
          const timeSinceFlash = now - lastFlash;
          const nextFlashDelay = 5000 + Math.random() * 5000; // 5-10 seconds
          
          if (timeSinceFlash > nextFlashDelay) {
            // Trigger flash
            setFlashActive(true);
            
            // Generate bolt after flash starts
            setTimeout(() => {
        ...
        const interval = setInterval(checkFlash, 100);
        return () => clearInterval(interval);
      }, [lastFlash]);
      
      // Generate lightning bolt using recursive branching
      const generateLightningBolt = () => {
        const startX = Math.random() * 100; // Random starting point at top
        const branches = [];
        
        const createBranch = (x, y, angle, length, depth) => {
          if (depth > 8 || length < 5) return; // Stop recursion
          ...
          createBranch(
            endX,
            endY,
            angle + (Math.random() - 0.5) * 0.3, // Slight zigzag
            length * 0.9,
            depth + 1
          );
        ...
        createBranch(startX, 0, Math.PI / 2, 15, 0); // Start pointing down
        return branches;
      };
      ...
      return (
        <div className="absolute inset-0 overflow-hidden" style={baseStyle}>
          ...
          <div
            className="absolute inset-0"
            style={{
              backgroundColor: flashActive ? '#ffffff' : 'transparent',
  ...
  if (showSettings) {
    return (
      <div className="min-h-screen flex flex-col relative overflow-hidden">
        <div
          className="absolute inset-0"
          style={{ background: 'linear-gradient(135deg, #00d2ff 0%, #3a7bd5 50%, #ff6b35 100%)' }}
        />
          ...
          <div className="p-6">
            ...
            <div className="mb-6">
              <h2 className="text-lg font-semibold mb-4 flex items-center space-x-2">
                ...
                <span>Wake Up Alarm Sounds</span>
              </h2>
                ...
                {Object.entries(alarmSounds).filter(([key, sound]) => !sound.hidden).map(([key, sound]) => (
                  <div key={key}>
                    ...
                    {selectedAlarmSound === 'classic-alarms' && key === 'classic-alarms' && (
                      <div className="mt-2 ml-4 p-3 bg-white/5 rounded-lg">
                        <label className="text-sm opacity-70 mb-2 block">Select Alarm Sound:</label>
                        <select
                          value={selectedClassicAlarm}
                          onChange={(e) => setSelectedClassicAlarm(e.target.value)}
                          className="w-full bg-white/10 text-white rounded-lg px-3 py-2 focus:bg-white/20 focus:outline-none"
                          onClick={(e) => e.stopPropagation()}
                        >
                          {classicAlarmLibrary.map((track) => (
                            <option key={track.fileName} value={track.fileName} className="bg-gray-800">
                              {track.displayName}
                            </option>
                          ))}
                        </select>
                    ...
                    {selectedAlarmSound === 'ambient-sounds' && key === 'ambient-sounds' && (
                      <div className="mt-2 ml-4 p-3 bg-white/5 rounded-lg">
                        <label className="text-sm opacity-70 mb-2 block">Select Ambient Sound:</label>
                        <select
                    ...
                    {selectedAlarmSound === 'natural-sounds' && key === 'natural-sounds' && (
                      <div className="mt-2 ml-4 p-3 bg-white/5 rounded-lg">
                        <label className="text-sm opacity-70 mb-2 block">Select Natural Sound:</label>
                        <select
                          ...
                          onClick={(e) => e.stopPropagation()}
                        >
                          <option value="rain-sounds" className="bg-gray-800">Rain Sounds</option>
                          <option value="ocean-waves" className="bg-gray-800">Ocean Waves</option>
                        </select>
                      </div>
                    )}
                  </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex-1">
                      <h3 className="text-sm font-semibold">Light Starts</h3>
                      <p className="text-xs opacity-70">20 minutes before alarm - gradual sunrise simulation</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex-1">
                      <h3 className="text-sm font-semibold">Sound Starts</h3>
                      <p className="text-xs opacity-70">30 seconds before alarm - gentle volume increase (30% → 100%)</p>
                    </div>
                  </div>

                  <div className="text-xs opacity-70 bg-blue-500/20 p-3 rounded mt-3">
            ...
            <div className="mb-6">
              <h2 className="text-lg font-semibold mb-4 flex items-center space-x-2">
                ...
                <span>Critical Alarm Permissions</span>
              </h2>
            ...
            {/* Nendo Animation Controls */}
            <div className="mb-6">
              <h2 className="text-lg font-semibold mb-4 flex items-center space-x-2">
                <span className="text-2xl">✨</span>
                <span>Nendo Animation Effects</span>
              </h2>
            ...
            <div className="p-3 bg-green-500/20 rounded-lg">
              <p className="text-sm text-green-200">
                <strong>🔊 Real Audio Enabled!</strong> Tap ▶ to hear actual sound previews.
                Ambient sounds will play for 3 seconds, alarms for 2 seconds.
              </p>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden">
      ...
      <div className="relative z-10 min-h-screen flex flex-col pb-24"
           style={{
             background: !isRunning ? (currentMode === 'wake' ? 'linear-gradient(to bottom, #e0f2fe 0%, #bae6fd 80%, #fef3c7 90%, #fed7aa 100%)' : 'linear-gradient(135deg, #00d2ff 0%, #3a7bd5 50%, #ff6b35 100%)') : 'transparent'
           }}
        ...
        <div className="p-6 pl-4">
          {currentMode === 'wake' ? (
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-white">LightAlarm</h1>
                  <p className="text-sm text-white opacity-80">Sleep Peacefully</p>
                </div>
              </div>
              <div className="flex items-center space-x-2">
                ...
                <button
                  ...
                  className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${
                    isComfortLightEnabled ? 'bg-amber-500/40 hover:bg-white/30' : 'bg-white/20 hover:bg-white/30'
                  }`}
                  title="Comfort Light"
                >
          ...
          {currentMode === 'sleep' && !isComfortLightEnabled && (
            <div className="p-2 bg-white/10 rounded-lg">
              <span className="text-xs mb-1.5 block opacity-80">Sleep Timer</span>
          ...
          {currentMode === 'sleep' && !isComfortLightEnabled && (
            <div className="p-3 bg-white/10 rounded-lg mt-2">
              <h3 className="text-xs font-medium opacity-80 mb-2">Sleep Theme</h3>
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-6 pl-4">
          ...
          <div className="relative mb-8">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <span className={`text-xl font-bold ${currentMode === 'wake' ? 'text-gray-900' : 'text-white'}`}>{Math.round(progress)}%</span>
            </div>
          </div>
