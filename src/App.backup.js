...
import { Sun, Moon, Play, Pause, RotateCcw, Clock, ChevronLeft, ChevronRight, Settings, Volume2, Lightbulb, Zap, Eye, EyeOff } from 'lucide-react';
...
const LightAlarm = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden">
      ...
      {/* Main Content */}
      <div className="relative z-10 min-h-screen flex flex-col text-white pb-24"
           style={{
             background: !isRunning ? 'linear-gradient(135deg, #00d2ff 0%, #3a7bd5 50%, #ff6b35 100%)' : 'transparent'
           }}
        ...
        <div className="p-6 pl-4">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-3">
              <div className="w-12 h-12 rounded-full bg-white/20 flex items-center justify-center">
                {currentMode === 'wake' ? <Sun className="w-6 h-6" /> : <Moon className="w-6 h-6" />}
              </div>
              <div>
                <h1 className="text-2xl font-bold">LightAlarm</h1>
                <p className="text-sm opacity-80">
                  {currentMode === 'wake' ? 'Wake Up Gently' : 'Sleep Peacefully'}
                </p>
              </div>
            </div>
            <div className="flex items-center space-x-2">
              ...
              <button
                ...
                className={`w-10 h-10 rounded-full flex items-center justify-center hover:bg-white/30 transition-all ${
                  isComfortLightEnabled ? 'bg-amber-500/40' : 'bg-white/20'
                }`}
                title="Comfort Light"
              >
          ...
          <div className="flex space-x-2 mb-6">
            ...
            >
              ...
              <span className="text-sm">Wake Mode</span>
            </button>
            <button
                ...
                {Object.entries(getCurrentThemes()).map(([key, theme]) => (
                  ...
                  >
                    ...
                    <div className="text-center">
                      <span className="text-xs font-medium block leading-tight">{theme.name}</span>
                    </div>
                  </button>
                ))}
              </div>
          ...
          {currentMode === 'wake' && (
            <div className="mb-6 space-y-4">
              ...
              <div className="p-3 bg-white/10 rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button
                    ...
                    className={`w-12 h-6 rounded-full transition-all ${
                      isAlarmEnabled ? 'bg-green-500' : 'bg-white/20'
                    }`}
                  >
                    <div className={`w-5 h-5 bg-white rounded-full transition-transform ${
                      isAlarmEnabled ? 'translate-x-6' : 'translate-x-0.5'
                    }`} />
                  </button>
                </div>
                {isAlarmEnabled && (
                  <div className="flex items-center justify-center space-x-2">
                    <div className="flex flex-col items-center">
                      <select
                        ...
                        className="w-16 h-12 bg-white/10 text-white rounded-lg text-center text-2xl font-mono cursor-pointer focus:bg-white/20 focus:outline-none"
                      >
                        {Array.from({length: 12}, (_, i) => i + 1).map(h => (
                          <option key={h} value={h.toString().padStart(2, '0')} className="bg-gray-800">
                            {h.toString().padStart(2, '0')}
                          </option>
                        ))}
                      </select>
                      <span className="text-xs opacity-60 mt-1">Hour</span>
                    </div>
                    ...
                    <div className="flex flex-col items-center">
                      ...
                      >
                        {Array.from({length: 60}, (_, i) => i).map(m => (
                          <option key={m} value={m.toString().padStart(2, '0')} className="bg-gray-800">
                            {m.toString().padStart(2, '0')}
                          </option>
                        ))}
                      </select>
                      <span className="text-xs opacity-60 mt-1">Minute</span>
                    </div>
                    ...
                    <div className="flex flex-col items-center ml-2">
                      <select
                        ...
                        className="w-14 h-12 bg-white/10 text-white rounded-lg text-center text-lg font-mono cursor-pointer focus:bg-white/20 focus:outline-none"
                      >
                        <option value="AM" className="bg-gray-800">AM</option>
                        <option value="PM" className="bg-gray-800">PM</option>
                      </select>
                      <span className="text-xs opacity-60 mt-1">AM/PM</span>
                    </div>
                  </div>
                )}
              </div>

              {/* 2. Theme Selector - Always visible */}
              <div className="p-3 bg-white/10 rounded-lg">
                <h3 className="text-xs font-medium opacity-80 mb-2">Wake Up Theme</h3>
                <div className="grid grid-cols-4 gap-1.5">
                  {Object.entries(getCurrentThemes()).map(([key, theme]) => (
                    <button
                      key={key}
                    ...
                    >
                      <div
                        className="w-full h-1 rounded-full mb-1 opacity-80"
                        style={{
                          background: `linear-gradient(90deg, ${theme.colors.slice(-3).join(', ')})`
                        }}
                      />
                      <div className="text-center">
                        <span className="text-xs font-medium block leading-tight">{theme.name}</span>
                ...
                <div className="p-4 bg-blue-500/20 rounded-lg border border-blue-400/30">
                  ...
                  <div className="text-xs space-y-1.5 opacity-90">
                    <div className="flex items-start">
                      ...
                      <span><strong>20 min before:</strong> Screen gradually brightens (sunrise simulation)</span>
                    </div>
                    <div className="flex items-start">
                      <span className="mr-2">🔊</span>
                      <span><strong>30 sec before:</strong> Alarm sound starts softly, increases to full volume</span>
                    </div>
                    <div className="flex items-start">
                      <span className="mr-2">⏰</span>
                      <span><strong>At alarm time:</strong> Full brightness + full volume wake-up</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              {isAlarmEnabled && (
                <div className="p-3 bg-white/10 rounded-lg">
                  <span className="text-sm mb-2 block">Repeat Days</span>
                      ...
                      <button
                        ...
                        className={`px-2 py-1 rounded text-xs transition-all ${
                          repeatDays.includes(day)
                            ? 'bg-green-500/80 shadow text-white'
              ...
              {isAlarmEnabled && (
                <button
                  ...
                  className="w-full py-2 bg-green-500 hover:bg-green-600 rounded-lg text-white text-xs font-medium transition-all"
                >
                  Save Alarm
                </button>
              )}
          ...
          <div className="flex space-x-4">
            {!isRunning ? (
              <button
                ...
                className="flex items-center space-x-2 bg-white/20 hover:bg-white/30 px-6 py-3 rounded-xl transition-all shadow-lg"
              >
                <Play className="w-5 h-5" />
                <span>Start {currentMode === 'wake' ? 'Wake Up' : 'Sleep'}</span>
              </button>
            ) : (
              <button
                onClick={stopSimulationWithConfirm}
            ...
            <button
              ...
              className="flex items-center space-x-2 bg-white/10 hover:bg-white/20 px-4 py-3 rounded-xl transition-all"
            >
